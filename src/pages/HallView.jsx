...
export default function HallView() {
  ...
  async function openSeat(num) {
    ...
    if (occ && occ.student_id) {
      ...
      setDetail(full);
    } else if (occ && occ.pending_application_id) {
      setSignups(await api.listSignups(slug).catch(() => []));
      setSignupsOpen(true);
    } else {
      setModalMode("empty");
    }
    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "18px 16px 60px" }}>
        <div style={{ display: "flex", gap: 16, marginBottom: 16, flexWrap: "wrap" }}>
          ...
          <LegendDot color="var(--pending)" label="Awaiting confirmation" />
      ...
      {reportsOpen && (
        <Modal onClose={() => setReportsOpen(false)}>
          <ReportsView reports={reports} hallName={hall?.name} slug={slug} />
        </Modal>
      )}
...
function ReportsView({ reports, hallName, slug }) {
  ...
  return (
    <div>
      ...
      <DateLookup slug={slug} />
      <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", color: "var(--brass-deep)", marginBottom: 6, marginTop: 18 }}>Fee paid — {reports.paidList.length}</div>
...
function DateLookup({ slug }) {
  ...
  async function lookup() {
    setBusy(true);
    try { setResult(await api.transactionsByDate(slug, date)); }
  ...
  return (
    <div className="card">
      <div style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 14, marginBottom: 8 }}>Check a specific date</div>
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <input type="date" className="field-input" value={date} onChange={(e) => setDate(e.target.value)} style={{ flex: 1 }} />
        <button onClick={lookup} disabled={busy} className="btn-primary">Go</button>
      </div>
      {result && result.error && <div className="err-box">{result.error}</div>}
      {result && !result.error && (
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Total confirmed: ₹{result.total}</div>
          <div style={{ maxHeight: 160, overflowY: "auto", border: "1px solid var(--line)", borderRadius: 8 }}>
            {result.transactions.length === 0 && <div style={{ padding: 10, fontSize: 12.5, color: "var(--ink-soft)" }}>No transactions on this date.</div>}
            {result.transactions.map((t) => (
              <div key={t.id} style={{ display: "flex", justifyContent: "space-between", padding: "7px 10px", fontSize: 12.5, borderBottom: "1px solid var(--line)" }}>
                <span>Seat {t.seat_number} — {t.student_name || "—"} ({t.type}, {t.method}, {t.status})</span>
                <span>₹{t.amount}</span>
              </div>
            ))}
          </div>
