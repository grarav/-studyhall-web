...
function AddStudentModal({ slug, seatNumber, onClose, onSaved, showToast }) {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <Field label="Student login password"><input type="password" className="field-input" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} placeholder="Leave blank to default to last 4 digits of mobile" /></Field>
      {form.password && (
        <>
