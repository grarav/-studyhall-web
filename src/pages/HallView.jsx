...
function UpiSettingsModal({ slug, hall, onClose, onSaved }) {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div style={{ fontSize: 12.5, color: "var(--ink-soft)", marginBottom: 14 }}>Payments now go through the real gateway (Razorpay), configured server-side. This UPI ID is kept for reference/QR display only.</div>
...
function AddStudentModal({ slug, seatNumber, onClose, onSaved, showToast }) {
  const [form, setForm] = useState({ name: "", mobile: "", aadhar: "", dob: "", gender: "", photoUrl: "", aadharPhotoUrl: "", password: "" });
  ...
  async function submit() {
    ...
    setErr(""); setBusy(true);
    try {
      await api.assignSeat(slug, seatNumber, { ...form, aadhar: form.aadhar, paymentMethod: "cash" });
      onSaved();
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <Field label="Photo"><PhotoUpload label="Take / upload photo" value={form.photoUrl} onChange={(url) => setForm((f) => ({ ...f, photoUrl: url }))} capture /></Field>
      ...
      <Field label="Mobile number *"><input className="field-input" value={form.mobile} onChange={(e) => setForm((f) => ({ ...f, mobile: e.target.value.replace(/\D/g, "").slice(0, 10) }))} /></Field>
      <Field label="Student login password"><input type="password" className="field-input" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} placeholder="Defaults to last 4 digits of mobile if blank" /></Field>
      ...
      <Field label="Aadhar card"><PhotoUpload label="Upload Aadhar card" value={form.aadharPhotoUrl} onChange={(url) => setForm((f) => ({ ...f, aadharPhotoUrl: url }))} /></Field>
