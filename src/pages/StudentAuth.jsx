...
function SignupWizard() {
  ...
  async function goToSeatStep() {
    ...
    setErr("");
    const res = await api.vacantSeats(form.hallSlug);
    setVacantSeats(res.seats);
    setUpiId(res.upiId);
    setStep("seat");
  }
  ...
  if (step === "seat") {
    return (
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8, maxHeight: 320, overflowY: "auto", marginBottom: 14 }}>
          {vacantSeats.map((s) => (
            <button
              key={s.seat_number}
              onClick={() => pickSeat(s)}
            ...
            >
              {s.seat_number}{s.is_locker ? <div style={{ fontSize: 9, color: "var(--brass-deep)", marginTop: 2 }}>locker</div> : null}
            </button>
          ))}
        </div>
        <button onClick={() => setStep("details")} className="link-btn">← Back</button>
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        <UpiQrPayment
          ...
          payeeName={form.hallSlug}
          note={`Seat ${selectedSeat.seat_number} - ${form.name}`}
          onConfirmed={afterPaid}
        />
