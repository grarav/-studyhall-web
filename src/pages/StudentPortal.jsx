          ...
          <>
            ...
            {changeOpen && (
              <div>
                ...
                <SeatChangePicker hallSlug={me.hall_slug} vacantSeats={vacantSeats} onDone={() => { setChangeOpen(false); load(); }} />
...
function RenewPanel({ me, months, setMonths, onClose, onDone }) {
  ...
  return (
    <div className="card" style={{ background: "rgba(92,138,102,0.08)" }}>
      ...
      <PaymentButton
        amount={amount} hallId={me.hall_id} studentId={me.id} seatNumber={me.seat_number} type="renewal"
        prefillName={me.name} prefillContact={me.mobile}
        onPaid={() => { alert("Payment received — your admin will confirm shortly."); onDone(); }}
      />
...
function SeatChangePicker({ hallSlug, vacantSeats, onDone }) {
  const plan = getFloorPlan(hallSlug);
  ...
  if (selected) {
    return (
      <PaymentButton
        amount={100} type="seat_change" label={`Pay ₹100 to move to seat ${selected}`}
        onPaid={async () => { await api.requestSeatChange({ toSeatNumber: selected }); alert("Sent to your admin for confirmation."); onDone(); }}
      />
    );
  }

  return <SeatGrid plan={plan} seatsByNumber={seatsByNumber} onlyVacant onSeatClick={setSelected} />;
}
